#!/bin/bash
# added by JIWON
# Parameters for the near-mem-ctrl PIM model
...
export NEW_PIM="TRUE"                       # JIWON: NEW_PIM set true only for near memory PIM
export HAVE_PIM_DEVICE="TRUE"
export NUM_PIM_DEVICES=8                    # JIWON: number of PIM devices for new PIM model (i.e. number of mem ports used as PIM)
export N_INIT_PORT=8                        # JIWON: number of mem ports used for host main mem
export DRAM_row_size="16"                   # bits; JIWON: in order to increase memory bank size (while keeping row buffer size constant)

export PIM_VAULT_BASE_ADDR="0xC0000000"     # JIWON: base physical addr of vaults set aside as PIM vaults. (needs to be x80000000 + mainmemsize)
...
#export PIM_HOST_OFFSET=0x$(printf "%08X" $(( $NUM_PIM_DEVICES * $PIM_SPM_SIZE )));
...
{
    echo -e "pim_print_mem_map not implemented yet for gem5_new_pim.sh"

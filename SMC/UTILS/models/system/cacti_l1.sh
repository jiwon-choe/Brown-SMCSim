...
echo -e "##### SCRIPT GENERATED BY ERFAN #####
# Cache size
-size (bytes) $(get_param $1.size)

# Line size
-block size (bytes) $(get_param $1.tags.block_size)

# To model Fully Associative cache, set associativity to zero
-associativity $(get_param $1.tags.assoc)
...
-UCA bank count 1
-technology (u) $CACHE_TECHNOLOGY_NODE_um
...
-output/input bus width $(($(get_param system.tol2bus.width)*8))

// 300-400 in steps of 10
-operating temperature (K) $OPERATING_TEMPERATURE
...
-design objective (weight delay, dynamic power, leakage power, cycle time, area) 0:0:100:0:0
...
# -- (Global_10, default, lowswing, fullswing)
...
-Core count $(get_param system.pim_sys.pim_memory.SYSTEM_NUM_CPU)

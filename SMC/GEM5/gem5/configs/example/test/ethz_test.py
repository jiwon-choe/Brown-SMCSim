...
# CONNECT ONE TRAFFIC GENERATOR TO MODELSIM INTERFACE
#
#  |-------|      |----------------|
#  | TGEN  |------| ethz_ModelsimIF |
#  |-------|      |----------------|
#
# In order to test the closed loop simulation interface
...
period = GEM5_TOTAL_SIMULATION_PERIOD  # ERFAN
...
system.cache_line_size=SMC_BURST_SIZE_B
...
system.monitor = [ CommMonitor() for i in xrange(np)]

# add a communication monitor

# connect the traffic generator to the bus via a communication monitor
for i in xrange(np):
	system.tgen[i].port = system.monitor[i].slave
	system.monitor[i].master = system.membus.slave
...
system.ctrl = ethz_ModelsimIF()
system.ctrl.REPORT_PERIOD_ps = GEM5_REPORT_PERIOD_ps
system.ctrl.SHORT_SLEEP_ns = GEM5_SHORT_SLEEP_ns
system.ctrl.MAX_INFLIGHT_TRANS = MAX_INFLIGHT_TRANS
system.ctrl.MAX_BUFFER_SIZE = GEM5_MAX_BUFFER_SIZE
system.ctrl.SYNCH_PERIOD_ps = GEM5_SYNCH_PERIOD_ps
system.ctrl.LONG_SLEEP_ns = GEM5_LONG_SLEEP_ns
system.ctrl.latency = '0ns';
system.ctrl.bandwidth = '500GB/s';

#### Direct Connection
system.membus.master = system.ctrl.port

### Bridge
#system.B = Bridge( req_size=512, resp_size=512, delay='0ns')
#system.membus.master = system.B.slave
#system.B.master = system.ctrl.port

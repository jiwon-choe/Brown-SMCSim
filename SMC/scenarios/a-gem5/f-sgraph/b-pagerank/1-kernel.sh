...
VALUES0=( default ) # dma1 dma2 dma3 coprocessor )
...
# 	load_model gem5_perf_sim.sh				# Fast simulation without debugging
	...
	export OFFLOADED_KERNEL_NAME=sgraph_page_rank   # Kernel name to offload (Look in SMC/SW/PIM/kernels)
    export OFFLOADED_KERNEL_SUBNAME=$V0
	
	#####################
    export OFFLOADED_GRAPH_NODES=1000             # Number of nodes
    export OFFLOADED_GRAPH_MAX_OUTDEGREE=10     # Maximum outdegree (sparse graph)
    export PAGERANK_MAX_ITERATIONS=100
    export PAGERANK_MAX_ERROR=0.001

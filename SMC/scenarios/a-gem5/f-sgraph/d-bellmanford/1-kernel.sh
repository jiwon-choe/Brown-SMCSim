...
VALUES0=( "TRUE" ) # "FALSE" ) # Use HMC Atomic commands or not
...
# 	load_model gem5_perf_sim.sh				# Fast simulation without debugging
	...
	export OFFLOADED_KERNEL_NAME=sgraph_bellman_ford   # Kernel name to offload (Look in SMC/SW/PIM/kernels)
    export OFFLOADED_KERNEL_SUBNAME=best
	
	#####################
    export OFFLOADED_GRAPH_NODES=2000           # Number of nodes
    export OFFLOADED_GRAPH_MAX_OUTDEGREE=10     # Maximum outdegree (sparse graph)
    export OFFLOADED_GRAPH_MAX_WEIGHT=10        # Maximum weight on the edges
	#####################
	#####################
	#####################
